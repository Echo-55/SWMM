...
class Ui_Options(QWidget):
    ...
    def setupUi(self):
        ...
        self.game_selection_box.currentTextChanged.connect(self._handle_game_selection_changed)
        self.game_selection_box.addItems(self.mod_downloader.config.get_game_list_from_config())
        ...
        self.layout_.addWidget(self.mode_help_button, 3, 0, 1, 2, QtCore.Qt.AlignmentFlag.AlignHCenter)
    ...
    def _handle_rename_mode_changed(self):
        ...
        if self.rename_mode_checkbox.isChecked():
            ...
            self.parent_window.downloader_tab.add_text_to_console('Rename mode enabled', color='green')
        else:
            self.rename_mode = False
            self.parent_window.downloader_tab.add_text_to_console('Rename mode disabled', color='red')
    ...
    def _handle_copy_mode_changed(self):
        ...
        if self.copy_mode_checkbox.isChecked():
            self.copy_mode = True
            self.parent_window.downloader_tab.add_text_to_console('Copy mode enabled', color='green')
        else:
            self.copy_mode = False
            self.parent_window.downloader_tab.add_text_to_console('Copy mode disabled', color='red')
...
class Ui_StatusBar(QtWidgets.QStatusBar):
    ...
    def refresh(self):
        ...
        """
        # self.steamcmd_installed = self.parent_window.steamcmd_installed
        # self.game_selected = self.parent_window.game_selected

        if self.steamcmd_installed:
            self.steamcmd_status_text.setText('Installed')
            self.steamcmd_status_text.setStyleSheet('color: green')
        else:
            self.steamcmd_status_text.setText('Not Installed')
            self.steamcmd_status_text.setStyleSheet('color: red')

        if self.game_selected:
            self.game_status_text.setText('Selected')
            self.game_status_text.setStyleSheet('color: green')
        else:
            self.game_status_text.setText('Not Selected')
            self.game_status_text.setStyleSheet('color: red')
    ...
    def setupUi(self):
        ...
        self.steamcmd_status_text = QtWidgets.QLabel(self)
        self.addWidget(self.steamcmd_status_text)
        if self.steamcmd_installed:
            self.steamcmd_status_text.setText('Installed')
            self.steamcmd_status_text.setStyleSheet('color: green')
        else:
            self.steamcmd_status_text.setText('Not Installed')
            self.steamcmd_status_text.setStyleSheet('color: red')
        # checkmark icon for good and x icon for bad
        ...
        if self.steamcmd_installed:
            self.steamcmd_status_icon.setPixmap(self.style().standardPixmap(checkmark_icon))
        else:
            self.steamcmd_status_icon.setPixmap(self.style().standardPixmap(x_icon))
        ...
        self.game_status_text = QtWidgets.QLabel(self)
        self.addWidget(self.game_status_text)
        if self.game_selected:
            self.game_status_text.setText('Selected')
            self.game_status_text.setStyleSheet('color: green')
        else:
...
class Ui_MenuBar(QtWidgets.QMenuBar):
    ...
    def _handle_about_action_triggered(self):
        ...
        """
        QtWidgets.QMessageBox.about(self, 'About', 'Mod Downloader\n\nA tool for downloading mods from the Steam Workshop')
...
class Ui_Downloader(QWidget):
    ...
    def setupUi(self):
        ...
        self.toggle_options_visibility_button = QtWidgets.QPushButton(icon=QtGui.QIcon('icons/arrow_down.png'), text='Show Options')
        self.toggle_options_visibility_button.setCheckable(True)
        self.toggle_options_visibility_button.clicked.connect(self._handle_toggle_options_visibility)
