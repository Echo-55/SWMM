...
class SteamCMD:
    ...
    def __init__(self, mod_downloader: "ModDownloader"):
        ...
        self.batch_size: int = int(
            self.config.get("DOWNLOADER", "batch_count", fallback=5)
        )
        ...
        self.steamcmd_path: str = self.config.get(
            "DEFAULT", "steamcmd_path", fallback=""
        )
        self.steamcmd_username: str = self.config.get(
            "DEFAULT", "steamcmd_username", fallback=""
        )
        self.steamcmd_password: str = self.config.get(
            "DEFAULT", "steamcmd_password", fallback=""
        )
    ...
    def run_steamcmd(self, args: list):
        ...
        if self.steamcmd_installed:
            ...
            proc = subprocess.Popen(
                args, stdout=subprocess.PIPE, stderr=subprocess.PIPE, errors="ignore"
            )
